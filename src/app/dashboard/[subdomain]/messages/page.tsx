...
export default function DashboardMessagesPage(props: any) {
  ...
  React.useEffect(() => {
    async function fetchSiteData() {
      ...
      if (error) {
        console.error("Error fetching site data for messages page:", error);
  ...
  if (loading) {
    return (
      <div className="container mx-auto">
        <h1 className="text-3xl font-bold mb-8 text-center lg:text-left">Chargement des Messages...</h1>
  ...
  return (
    <div className="container mx-auto">
      <h1 className="text-3xl font-bold mb-8 text-center lg:text-left">Messages de {site.site_data?.publicName || site.subdomain}</h1>
      <div className="max-w-3xl mx-auto lg:mx-0 space-y-8">
        <MessagesList siteId={site.id} />
      </div>
    </div>
  );
}